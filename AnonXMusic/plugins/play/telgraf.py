...
@app.on_message(filters.command(["تلجراف", "تلجراف ميديا", "ميديا"], "") )
...
@app.on_message(filters.command(["معلوماته", "هوو"], "") & filters.group ) 
async def hshs(client: Client, message: Message):      
    ...
    async for photo in client.iter_profile_photos(message.reply_to_message.from_user.id, limit=1):
                    await message.reply_photo(photo.file_id,       caption=f"""**[࿈ 𝐒𝐎𝐔᚜𝐀𝐋𝟑𝐍𝟖𝟐᚛𝐑𝐂𝐄 ࿈](https://t.me/SOURCEVEGA)\n\n🖤 ¦ ɴᴀᴍᴇ : {name}\n🕸 ¦ ᴜѕᴇʀ : {username}\n🕷 ¦ ɪᴅ : `{user_id}`\n🔥 ¦ ɪᴅ ᴄʜᴀᴛ : `{chat_idd}`\n🏴‍☠ ¦ ᴄʜᴀᴛ : {chat_name}\n🐊 ¦ ɢʀᴏᴜᴘ : {chat_username} \n**""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
...
@app.on_message(filters.command(["تحويل لصوره", "تحويل الصوره"]))
...
@app.on_message(filters.command(["الجروب", "جروب","انا فين"]) & filters.group )
