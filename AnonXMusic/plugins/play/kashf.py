...
async def hshs(client: Client, message: Message):      
    ...
    async for photo in client.iter_profile_photos(message.reply_to_message.from_user.id, limit=1):
                    await message.reply_photo(photo.file_id,       caption=f"""🖤 ¦ ꪀᥲ️ꪔᥱ : {name}\n🤡 ¦ ᴜѕᴇ : {username}\n🔥 ¦ Ꭵَժ : <code>{user_id}</code>\n🗿 ¦ Ꭵժ ᥴ𝗁ᥲ️ƚ : <code>{chat_idd}</code>\n🐰 ¦ 𝚌𝚑𝚊𝚝 : {chat_name}\n🐊 ¦ ᘜᖇ᥆υρ : {chat_username} \n""", 
...
@app.on_message(filters.command(["تحويل", "تحويل الصوره"], ""))
